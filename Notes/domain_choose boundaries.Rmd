---
title: "Choosing StrathE2E domain extent - Canary Current"
...
For the Canary current we have inspected the distribution of vertical diffusivity values in the region from NEMO-MEDUSA model output. 60 m looks like a sensible depth to separate the shallow and deep layers.

** Following this paper http://dx.doi.org/10.1016/j.jmarsys.2009.10.007 we updated the boundary to 100m to capture the deeper chlorophyll for the region. **
...
Finally we need to "cut" the polygons defined by depths and distances from shore so that they don't extend around the whole world. Our first suggestion is to use the EEZ for the Canary Isles. This removes the continental shelf along Africa from the Model domain, and captures fishing activity on offshore banks. If there are reasons to choose different breaks, please let us know.
...
The model domain is shown in grey with white borders, while the fishing effort is shown as a heatmap on a log scale. By using the EEZ to define the model domain, we appear to capture all relevant fishing activity. If there is a reason for the offshore zone to be smaller than the EEZ then let us know.
