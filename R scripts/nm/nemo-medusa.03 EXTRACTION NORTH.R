...
# saveRDS("./Objects/Months NORTH/.")  # Marker so network script can see where the data is being saved too, it's buried in a function
...
#  .[1:12] %>% 
  future_map(NEMO_MEDUSA, analysis = "slabR", summary = scheme_result,
             ...
             count = count, out_dir = "./Objects/Months NORTH", .progress = T)    # Perform the extraction and save an object for each month (in parallel)
tictoc::toc()
