...
# saveRDS("./Objects/Months SOUTH/.")  # Marker so network script can see where the data is being saved too, it's buried in a function
...
crop <- readRDS("./Objects/Domains.rds") %>%                                # Load SF polygons of the MiMeMo model domains
  st_buffer(dist = 3) %>%                                                   # It needs to be a bit bigger for sampling flows at the domain boundary
...
#  .[1:12] %>% 
  future_map(NEMO_MEDUSA, analysis = "slabR", summary = scheme_result,
             ...
             count = count, out_dir = "./Objects/Months SOUTH", .progress = T)    # Perform the extraction and save an object for each month (in parallel)
toc()
