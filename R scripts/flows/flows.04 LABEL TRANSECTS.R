...
ggplot() +                                                              # Check the polygons match correctly with points
  geom_sf(data = grid, aes(fill = Temperature), size = 0.05, colour = "white") +
...
labelled <- st_intersection(Edges, grid) %>% 
  ...
  filter(split_length > 1) %>%                                              # Drop small transects that break code
  select(x, y, slab_layer, Shore, split_length, Bathymetry) %>% 
  characterise_flows(domains, precision = 10000000) %>%                       # In which direction? (in or out of box and with which neighbour)
